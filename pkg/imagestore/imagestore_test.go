...
var _ = Context("with a data directory configured", func() {
	...
	Context("with a test server", func() {
		...
		Describe("Populate", func() {
			...
			Context("s390x architecture support", func() {

				It("handles s390x architecture with cached nmstatectl", func() {
					...
					// No CreateMinimalISOTemplate should be called for s390x
					mockEditor.EXPECT().CreateMinimalISOTemplate(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(0)

					Expect(is.Populate(ctx)).To(Succeed())

					// Verify nmstatectl cache exists
					_, err = os.Stat(nmstatectlPath)
					Expect(err).NotTo(HaveOccurred())
				})
			})

			Context("nmstatectl extraction and caching", func() {
				DescribeTable("caches nmstatectl and creates minimal ISO for supported architectures",
					func(arch string) {
						...
						// Verify nmstatectl cache exists
						content, err := os.ReadFile(nmstatectlPath)
						Expect(err).NotTo(HaveOccurred())
						Expect(content).To(Equal([]byte(cacheContent)))
					},
				...
				It("skips nmstatectl extraction for versions that don't support it", func() {
					...
					// Verify nmstatectl was not cached
					_, err = os.Stat(nmstatectlPath)
					Expect(err).To(HaveOccurred())
					Expect(os.IsNotExist(err)).To(BeTrue())
				})

				It("handles invalid version format errors", func() {
					version := map[string]string{
						"openshift_version": "invalid.version.format", // Invalid version that can't be parsed
						"cpu_architecture":  "x86_64",
						"version":           "418.84.202109241901-0",
						"url":               ts.URL() + "/some.iso",
					}
					...
					err = is.Populate(ctx)
					Expect(err).To(HaveOccurred())
					Expect(err.Error()).To(ContainSubstring("failed to extract and cache nmstatectl for invalid.version.format-x86_64"))
				})
