...
// updateKernelArgs applies the standard kernel argument transformations:
// 1. Remove coreos.liveiso parameter
// 2. Add coreos.live.rootfs_url parameter at the specified insertion point
func updateKernelArgs(content string, insertionPattern string, rootFSURL string, fileEntry *kargsFileEntry) (string, error) {
	// Validate rootfs URL
...
func fixGrubConfig(rootFSURL, extractDir string, includeNmstateRamDisk bool, kargs *kargsConfig) error {
	...
	contentStr, err = updateKernelArgs(contentStr, `(?m)^(\s+linux .+)(?P<replace>$)`, rootFSURL, fileEntry)
	if err != nil {
		return err
	}
...
func fixIsolinuxConfig(rootFSURL, extractDir string, includeNmstateRamDisk bool, kargs *kargsConfig) error {
	...
	// Apply standard kernel argument transformations (remove coreos.liveiso, add rootfs_url)
	contentStr, err = updateKernelArgs(contentStr, `(?m)^(\s+append .+)(?P<replace>$)`, rootFSURL, fileEntry)
	if err != nil {
		return err
	}
...
func updateDefaultKargs(config *kargsConfig, rootFSURL string) error {
	...
	var err error
	config.Default, err = updateKernelArgs(config.Default, `(?P<replace>$)`, rootFSURL, nil)
	if err != nil {
		return err
	}
