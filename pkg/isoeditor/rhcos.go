...
func updateKernelArgs(filePath string, linePattern string, rootFSURL string) error {
	// Validate rootfs URL
...
func fixGrubConfig(rootFSURL, extractDir string, includeNmstateRamDisk bool) error {
	...
	if err := updateKernelArgs(foundGrubPath, `(?m)^(\s+linux) (.+| )+$`, rootFSURL); err != nil {
		return err
	}
...
func fixIsolinuxConfig(rootFSURL, extractDir string, includeNmstateRamDisk bool) error {
	...
	// Apply standard kernel argument transformations (remove coreos.liveiso, add rootfs_url)
	if err := updateKernelArgs(isolinuxPath, `(?m)^(\s+append) (.+| )+$`, rootFSURL); err != nil {
		return err
	}
